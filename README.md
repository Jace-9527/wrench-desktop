...
转换成功后会自动保存历史。当前核心存储使用 SQLite，支持按工具过滤、搜索、分页加载和收藏置顶。Wails 服务会把数据库放在用户配置目录：

- macOS: `~/Library/Application Support/Wrench Desktop/wrench.db`
- Linux: `${XDG_CONFIG_HOME:-~/.config}/Wrench Desktop/wrench.db`
- Windows: `%AppData%\Wrench Desktop\wrench.db`
